...
const AdjectiveDetail = () => {
  ...
  if (loading) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-teal-50 via-white to-rose-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading Adjective lesson...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-orange-50">
      <div className="bg-gradient-to-r from-pink-600 to-orange-600 text-white sticky top-0 z-50 shadow-lg">
        <div className="container mx-auto max-w-6xl px-4 py-6">
          <button onClick={() => navigate(-1)} className="mb-3 flex items-center space-x-1 text-white hover:text-pink-100 transition-colors text-sm">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm md:text-base text-pink-100">Master {data.name.toLowerCase()} with examples and practice</p>
              </div>
            </div>

            <div className="flex flex-wrap gap-2">
              {sections.map(section => (
                <button key={section.id} onClick={() => scrollToSection(section.id)} className={`px-3 py-1.5 rounded-full text-xs md:text-sm font-semibold transition-all ${activeSection === section.id ? 'bg-white text-pink-600 shadow-md' : 'bg-pink-500 bg-opacity-40 text-white hover:bg-opacity-60'}`}>
      ...
      <div className="container mx-auto max-w-6xl px-4 py-8 md:py-12">
        <section id="overview" className="mb-12 scroll-mt-32">
          <div className="bg-gradient-to-r from-slate-50 to-pink-50 rounded-xl shadow-sm border border-slate-200 p-3 md:p-4 mb-6 hover:shadow-md transition-all">
            <div className="flex items-start gap-2 md:gap-3 mb-2">
              <span className="text-xl md:text-2xl flex-shrink-0 pt-0.5">📖</span>
              <div className="flex-1 min-w-0">
                <h2 className="text-base md:text-lg font-bold text-slate-800 leading-tight">What is an {data.name}?</h2>
              </div>
            </div>

            <p className="text-xs md:text-sm text-slate-700 leading-snug mb-2 pl-7 md:pl-8">{data.definition}</p>

            <div className="grid grid-cols-2 gap-2 pl-0">
              <div className="bg-white border border-pink-200 rounded-lg p-2 md:p-2.5">
                <p className="text-xs md:text-xs font-semibold text-pink-700">💡 Why Learn?</p>
                <p className="text-xs text-slate-600 leading-tight mt-0.5">{data.importance}</p>
              </div>
              <div className="bg-white border border-orange-200 rounded-lg p-2 md:p-2.5">
                <p className="text-xs md:text-xs font-semibold text-orange-700">🎯 Key Point</p>
                <p className="text-xs text-slate-600 leading-tight mt-0.5">Adjectives add detail and create vivid descriptions.</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-10">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto">
              {data.types.map((type) => (
                <div key={type.id} className="bg-gradient-to-br from-pink-50 to-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 border border-pink-200 overflow-hidden flex flex-col h-full">
                  <div className="bg-gradient-to-r from-pink-100 to-pink-50 px-4 py-3 border-b-2 border-pink-200">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-2xl">{type.icon}</span>
                      <h3 className="text-base font-bold text-pink-800">{type.name}</h3>
                    </div>
                  </div>
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-1 mt-auto">
                      {type.sample_words?.slice(0, 4).map((w, widx) => (
                        <span key={widx} className="bg-pink-100 text-pink-700 px-2 py-0.5 rounded-full text-sm font-medium">{w}</span>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {data.resources.filter(r => r.resource_type === 'video').slice(0, 2).map(video => (
                  <div key={video.id} className="bg-gradient-to-br from-pink-50 to-orange-50 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all">
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm space-y-4">
                {data.exercises.filter(e => e.exercise_type === 'writing').map(exercise => (
                  <div key={exercise.id}>
                    ...
                    {writingSubmitted && (<div className="mt-4 bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg animate-fade-in"><p className="text-blue-800 font-semibold">✓ Submitted!</p></div>)}
        ...
        {data.quiz && data.quiz.length > 0 && (
          <section id="quiz" className="mb-16 scroll-mt-32">
            <div className="text-center mb-8">
              ...
              <button onClick={() => { setShowQuizModal(true); setCurrentQuestionIndex(0); setModalQuizAnswers({}); setSingleQuestionMode(false); }} className="inline-block bg-gradient-to-r from-pink-600 to-orange-600 text-white font-bold py-3 px-8 rounded-lg shadow-lg transition-all">▶️ Start Full Quiz</button>
            </div>
              ...
              {data.quiz.map((question, qIndex) => {
                ...
                return (
                  <div key={question.id} onClick={() => { setShowQuizModal(true); setCurrentQuestionIndex(qIndex); setModalQuizAnswers({}); setSingleQuestionMode(true); }} className="bg-white rounded-lg p-3 shadow-md border border-gray-200 hover:shadow-lg hover:border-pink-400 cursor-pointer transition-all duration-200 transform hover:scale-105">
                    ...
                    {answered ? (<div className={`text-xs font-medium p-1.5 rounded ${answered.correct ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-orange-50 text-orange-700 border border-orange-200'}`}>{answered.correct ? 'Correct!' : 'Try again'}</div>) : (<div className="text-xs text-pink-600 font-medium">Click to attempt</div>)}
                  </div>
                );
              })}
            </div>
            ...
            {showQuizModal && (
              <div className="fixed inset-0 bg-gradient-to-br from-slate-50 via-white to-slate-50 z-50 overflow-y-auto flex items-center justify-center p-3 md:p-6">
                <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-xl">
                  {!singleQuestionMode && (
                    <div className="h-1.5 bg-slate-100"><div className="h-full bg-gradient-to-r from-pink-500 to-orange-500 transition-all" style={{ width: `${((currentQuestionIndex + 1) / data.quiz.length) * 100}%` }} /></div>
                  )}
                  ...
                  <div className={`px-6 md:px-8 py-4 md:py-5 border-b border-slate-100 flex items-center justify-between ${singleQuestionMode ? 'bg-slate-50' : ''}`}>
                    ...
                    <div className="flex items-center gap-4 md:gap-6">
                      {!singleQuestionMode && (<div className="text-right"><div className="text-lg md:text-xl font-bold text-pink-600">{Object.values(modalQuizAnswers).filter(a => a.correct).length * 10}</div><div className="text-xs text-slate-500">points</div></div>)}
                  ...
                  {currentQuestionIndex < data.quiz.length ? (
                    ...
                    <div className="p-6 md:p-8 text-center space-y-5">
                      ...
                      {Object.keys(modalQuizAnswers).length === data.quiz.length && (
                        <>
                          ...
                          <div className="grid grid-cols-3 gap-2 md:gap-3">
                            ...
                            <div className="bg-blue-50 p-3 md:p-4 rounded-lg border border-blue-200 text-center"><p className="text-2xl md:text-3xl font-bold text-blue-600 mb-1">{Math.round((Object.values(modalQuizAnswers).filter(a => a.correct).length / data.quiz.length) * 100)}%</p><p className="text-xs md:text-sm font-semibold text-slate-600">Accuracy</p></div>
                          </div>
                        </>
                      )}
                      ...
                      <div className="flex flex-col sm:flex-row gap-2.5 pt-4 border-t border-slate-100">
                        ...
                        <button onClick={() => { setCurrentQuestionIndex(0); setModalQuizAnswers({}); }} className="flex-1 px-4 py-2.5 rounded-lg bg-gradient-to-r from-pink-500 to-orange-500 text-white font-medium text-sm md:text-base hover:shadow-lg transition-all">🔄 Restart Quiz</button>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {data.resources.slice(0, 6).map((resource) => (
                  <a key={resource.id} href={resource.url} target="_blank" rel="noopener noreferrer" className="bg-gradient-to-br from-pink-50 to-pink-100 rounded-lg p-4 border border-pink-300 hover:shadow-md transition-all">
                    <span className="text-2xl block mb-2">📖</span>
                    <h3 className="font-semibold text-pink-700 text-base mb-1">{resource.title}</h3>
        ...
        <div className="bg-gradient-to-r from-pink-600 to-orange-600 rounded-xl shadow-md p-6 text-white text-center">
          <h3 className="text-xl font-bold mb-2">🎓 Ready to practice?</h3>
          <p className="text-sm mb-4 text-pink-100">Try quizzes and exercises to sharpen your skills.</p>
          <div className="flex flex-wrap gap-4 justify-center">
            <button onClick={() => navigate('/modules/parts-of-speech')} className="bg-white text-pink-600 px-8 py-3 rounded-lg font-semibold hover:bg-pink-50 transition-colors shadow-lg">All Parts</button>
...
export default AdjectiveDetail;
