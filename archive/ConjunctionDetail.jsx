...
const ConjunctionDetail = () => {
  ...
  const partId = 15; // Conjunction ID
  ...
  const fetchPartData = async () => {
    try {
      setLoading(true);
      // Fetch part details (Conjunctions)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-rose-50">
      <div className="bg-gradient-to-r from-red-600 to-rose-600 text-white sticky top-0 z-50 shadow-lg">
        <div className="container mx-auto max-w-6xl px-4 py-6">
          <button onClick={() => navigate(-1)} className="mb-3 flex items-center space-x-1 text-white hover:text-red-100 text-sm">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-red-100">Master {data.name.toLowerCase()}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto max-w-6xl px-4 py-8 md:py-12">
        <section id="overview" className="mb-12">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-6 text-center">Key {data.name} Topics</h2>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {data.types.map((type) => (
              <div key={type.id} className="bg-gradient-to-br from-red-50 to-white rounded-xl shadow-md border border-red-200 p-4">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-2xl">{type.icon}</span>
                  <h3 className="text-base font-bold text-red-800">{type.name}</h3>
                </div>
                <p className="text-sm text-gray-700 mb-3">{type.description}</p>
                <div className="flex flex-wrap gap-1">
                  {type.sample_words?.slice(0, 3).map((w, widx) => (
                    <span key={widx} className="bg-red-100 text-red-700 px-2 py-0.5 rounded-full text-xs font-medium">{w}</span>
                  ))}
                </div>
              </div>
            ))}
        ...
        {data.quiz && data.quiz.length > 0 && (
          <section id="quiz" className="mb-16">
            <div className="text-center mb-8">
              ...
              <button onClick={() => { setShowQuizModal(true); setCurrentQuestionIndex(0); setModalQuizAnswers({}); }} className="inline-block bg-gradient-to-r from-red-600 to-rose-600 text-white font-bold py-3 px-8 rounded-lg shadow-lg">▶️ Start Quiz</button>
            </div>
              ...
              {data.quiz.map((question, qIndex) => {
                ...
                return (
                  <div key={question.id} onClick={() => { setShowQuizModal(true); setCurrentQuestionIndex(qIndex); setModalQuizAnswers({}); }} className="bg-white rounded-lg p-3 shadow-md border border-gray-200 hover:shadow-lg cursor-pointer transition-all">
                    ...
                    {answered ? (<div className={`text-xs font-medium p-1.5 rounded mt-2 ${answered.correct ? 'bg-green-50' : 'bg-orange-50'}`}>{answered.correct ? 'Correct!' : 'Try again'}</div>) : (<div className="text-xs text-red-600 font-medium mt-2">Click to attempt</div>)}
                  </div>
                );
              })}
            </div>
            ...
            {showQuizModal && data.quiz && (
              <div className="fixed inset-0 bg-slate-50 z-50 overflow-y-auto flex items-center justify-center p-3">
                <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl">
                  <div className="h-1.5 bg-red-300" style={{ width: `${((currentQuestionIndex + 1) / data.quiz.length) * 100}%` }}></div>
                  ...
                  {currentQuestionIndex < data.quiz.length ? (
                              ...
                              {question.options.map((option, index) => (
                                <button key={index} onClick={() => { if (!answered) { setModalQuizAnswers(prev => ({ ...prev, [question.id]: { selected: index, correct: index === question.correct_answer } })); } }} disabled={answered} className={`w-full p-3 rounded-lg border-2 transition-all text-left font-medium text-sm ${answered ? index === question.correct_answer ? 'bg-green-50 border-green-400' : answered.selected === index ? 'bg-red-50 border-red-400' : 'bg-slate-50 border-slate-200' : 'bg-white border-slate-300 hover:border-red-400 hover:bg-red-50 cursor-pointer'}`}>
                                  <div className="flex items-center gap-2">
                                    <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full font-bold text-xs ${answered ? index === question.correct_answer ? 'bg-green-200' : answered.selected === index ? 'bg-red-200' : 'bg-slate-200' : 'bg-red-100 text-red-600'}`}>{String.fromCharCode(65 + index)}</span>
                                    <span>{option}</span>
                                  </div>
                                </button>
                              ))}
                    ...
                    <div className="p-6 text-center">
                      ...
                      <button onClick={() => { setCurrentQuestionIndex(0); setModalQuizAnswers({}); }} className="w-full px-4 py-2 rounded-lg bg-gradient-to-r from-red-500 to-rose-500 text-white font-medium">🔄 Restart</button>
                    </div>
                  )}
                </div>
              </div>
...
export default ConjunctionDetail;
